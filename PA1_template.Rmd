...
```{r plot2code, echo=TRUE, message=FALSE}
        require(dplyr)
        require(ggplot2)
        ad5 <- ad
...
```{r impute, echo=TRUE, message=FALSE, results=FALSE, cache=TRUE}
        require(mice)
        imp <- mice(ad)
        ad6 <<- complete(imp)
...
```{r wday,echo=TRUE, message=FALSE, results=FALSE, cache=TRUE}
        require(dplyr)
        require(ggplot2)
        require(mice)
        imp <- mice(ad)
